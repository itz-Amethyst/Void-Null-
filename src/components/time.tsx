...
import {daysUntilBirthday, IRTimeFormatter} from '../util/time';
...
function Night({time}: {time: Date}) {
	...
	React.useEffect(() => {
		...
		for (let i = 0; i < 50; i++) {
			ctx.fillStyle = 'white';
			ctx.beginPath();
			ctx.arc(Math.random() * canvas.width, Math.random() * canvas.height, Math.random() * 3, 0, Math.PI * 12);
			ctx.fill();
		}
	}, [canvasRef]);
	...
	return (
		...
		>
			...
			<div className="z-10 text-center">
				...
				>
					{IRTimeFormatter.format(time)}
				</h2>

				<p className={clsx('text-xs font-light', 'text-glow-sky-900 dark:text-glow-sky-500')}>in the IR</p>
			</div>
		</div>
	);
}
			...
			<div className="flex w-full items-center justify-center">
				<div className="opacity-85 flex items-baseline space-x-1 md:space-x-2">
					<div className="flex flex-col">
						<div className="font-title text-center text-base text-white">{IRTimeFormatter.format(time)}</div>
						<p
							className={clsx('text-center text-s font-light', 'text-white text-glow-sky-900 dark:text-glow-sky-500')}
						>
							in the IR
						</p>
					</div>
...
export function Time() {
	...
	return (
		<div className="grid grid-cols-1 gap-6 md:col-span-1 col-span-2">
